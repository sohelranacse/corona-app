...
def getComparisonData(request):
    ...
    for country in countryList:
        df = pd.read_json('http://wuhan-coronavirus-api.laeyoung.endpoint.ainize.ai/jhu-edu/timeseries?onlyCountries=true&iso3='+country+'').timeseries
...
def getPredictionData(request):
    ...
    # make dataframe
    df['Date'] = pd.to_datetime(df['Date'])
    data = pd.DataFrame(columns = ['Date','actual_Date','New_Deaths','New_Confirmed','New_Recovered'])

    data['Date'] = df['Date']
    data['actual_Date'] = data['Date'].dt.strftime('%Y-%m-%d') # for tracer
    data['New_Confirmed'] = df['New_Confirmed'].values
    data['New_Deaths'] = df['New_Deaths'].values
    data['New_Recovered'] = df['New_Recovered'].values

    ### confirmed cases
    # model apply
    arima = ARIMA(data['New_Confirmed'], order=(5, 1, 0))
    ...
    prediction_new_confirmed_layout = {
        'title': {
            'text': 'Coronavirus Confirmed Cases Next 14 Days Prediction - '+country,
            'font': {
                'color': '#000',
                'size': 14
            }
    ...
    ### deaths cases
    # model apply
    arima = ARIMA(data['New_Deaths'], order=(5, 1, 0))
    arima = arima.fit(trend='c', full_output=True, disp=True)
    forecast = arima.forecast(steps= 14)
    ...
    prediction_deaths_layout = {
        'title': {
            'text': 'Coronavirus Deaths Next 14 Days Prediction - '+country,
            'font': {
                'color': '#000',
                'size': 14
            }
    ...
    ### recovered cases
    # model apply
    arima = ARIMA(data['New_Recovered'], order=(5, 1, 0))
    arima = arima.fit(trend='c', full_output=True, disp=True)
    forecast = arima.forecast(steps= 14)
    Predict_New_Recovered_data = list(forecast[0]) # y values

    # recovered new deaths chart start    
    prediction_recovered_traccer1 = {
        'x': list(data["actual_Date"]),
        'y': list(data["New_Recovered"]),
        'type': 'scatter',
        'name': 'Actual',
        'marker': {
            'color': '#2DCE99',
    ...
    prediction_recovered_traccer2 = {
        'x': predictedDate,
        'y': Predict_New_Recovered_data,
        'type': 'scatter',
        'name': 'Predicted',
        'mode': 'lines+markers',
        'marker': {
    ...
    prediction_recovered_layout = {
        'title': {
            'text': 'Coronavirus Recovered Next 14 Days Prediction - '+country,
            'font': {
                'color': '#000',
                'size': 14
            }
    ...
    json_data = json.dumps({
        ...
        "prediction_recovered_traccer1": prediction_recovered_traccer1,
        "prediction_recovered_traccer2": prediction_recovered_traccer2,
        "prediction_recovered_layout": prediction_recovered_layout
